...
def install():
    ...
    with lcd(magento_root):
        local('n98-magerun.phar install --installationFolder . --noDownload --dbHost="localhost" --dbUser="root" --dbPass="" --dbName="mage_local" --dbPort="3306" --no-interaction --baseUrl="%s"' % config.get('base_url', 'http://magento.local/'))
...
def generate_composer_json():
    ...
    composer_json_location = os.path.join('/tmp/composer.json')
    ...
    with lcd(project_root):
        ...
        #
        # We set deploystrategy to copy, so that we don't have to
        # include the vendor directory for debugging.
        #
...
def install_module():
    ...
    """
    composer_json_location = '/tmp/composer.json'
    composer_json = json.load(open(composer_json_location))

    mappings = composer_json['extra'].get('map', [])

    for mapping in mappings:
        dest_abs = os.path.join(magento_root, mapping[1])
        src_abs = os.path.join(project_root, mapping[0])

        create_relative_link(src_abs, dest_abs)

def create_relative_link(src_abs, dest_abs):
    """Take two relative links and make a relative symlink."""
    with lcd(project_root):
        src = os.path.relpath(src_abs, os.path.dirname(dest_abs))
        local('mkdir -p %s' % os.path.dirname(dest_abs))
        local('ln -s %s %s' % (src, dest_abs))
